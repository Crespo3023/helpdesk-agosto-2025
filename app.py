...
def get_current_user():
    """Return the current logged-in user dict from the DB or None."""
    ...
    try:
        ...
        # If DB access fails treat as not logged in
        return None
@app.route("/")
def index():
    # If user is logged in and exists in DB, redirect to dashboard.
    # If the session is stale (user deleted), clear session and show login form.
    user = get_current_user()
    if user:
        # ensure session has up-to-date name/role
        session["user_name"] = user.get("name")
        session["user_role"] = user.get("role")
        return redirect(url_for("dashboard"))
    # clear any stale session info and show login form
...
def dashboard():
    ...
    if not user:
        ...
        return render_template("login.html")
    # make sure session info is synced
    session["user_name"] = user.get("name")
    session["user_role"] = user.get("role")
